...
public class FascicoloExcelBuilder {
	private static Logger logger = Logger.getLogger(FascicoloExcelBuilder.class.getCanonicalName());

    @SuppressWarnings("unchecked")
    public byte[] buildExcelDocuments(List<FascicoloVO> elenco, HSSFWorkbook workbook) throws Exception {

    String pattern = "dd/MM/yyyy";

  	// create a new Excel sheet
  	Sheet sheet = workbook.createSheet("Report fascicoli");
  	...
  	// CELL STYLE GENERICO, DI DEFAULT ALLINEAMENTO CENTRO
  	HSSFCellStyle styleValDefAllCenter = ((HSSFWorkbook) workbook).createCellStyle();
  	font = workbook.createFont();
  	font.setFontName("Arial");
  	styleValDefAllCenter.setWrapText(true);
  	styleValDefAllCenter.setVerticalAlignment(VerticalAlignment.TOP);
  	styleValDefAllCenter.setBorderBottom(BorderStyle.THIN);
  	styleValDefAllCenter.setBorderTop(BorderStyle.THIN);
  	styleValDefAllCenter.setBorderLeft(BorderStyle.THIN);
  	styleValDefAllCenter.setBorderRight(BorderStyle.THIN);
  	styleValDefAllCenter.setAlignment(HorizontalAlignment.CENTER);
  	font.setColor(HSSFColor.HSSFColorPredefined.BLACK.getIndex());
  	styleValDefAllCenter.setFont(font);
  	...
  	col++;
  	header0.createCell(col).setCellValue("Codice fascicolo");
  	...
  	col++;
  	header0.createCell(col).setCellValue("Indirizzo");
  	header0.getCell(col).setCellStyle(style);
  	
  	col++;
  	header0.createCell(col).setCellValue("Intestatario");
  	header0.getCell(col).setCellStyle(style);

  	col++;
  	header0.createCell(col).setCellValue("Stato");
  	header0.getCell(col).setCellStyle(style);
  	
  	col++;
  	header0.createCell(col).setCellValue("Data Stato");
  	header0.getCell(col).setCellStyle(style);
  	// create data rows
  	int rowCount = 1;
  	int countCol = -1;
  	for (FascicoloVO item : elenco) {
  	    countCol = -1;
  	    Row aRow = sheet.createRow(rowCount++);

  	    countCol++;
        aRow.createCell(countCol).setCellValue(item.getCodiceFascicolo());
        ...
        if(item.getUbicazione() != null && item.getUbicazione().getDug() != null && item.getUbicazione().getDug().getDescrizione() != null ) {
        	indirizzo.append(item.getUbicazione().getDug().getDescrizione());
        	indirizzo.append(" ");
        }
        if(item.getUbicazione() != null  && item.getUbicazione().getDuf() != null ) {
        	indirizzo.append(item.getUbicazione().getDuf());
        	indirizzo.append(" ");
        }
        if(item.getUbicazione() != null  && item.getUbicazione().getNumero() != null ) {
        	indirizzo.append(item.getUbicazione().getNumero());
        }
        ...
        countCol++;
  	    String intestatario= "";
  	    if(item.getIntestatario() != null && item.getIntestatario() != null && item.getIntestatario().getAnagrafica()!=null && item.getIntestatario().getAnagrafica().getCognome() != null)
  	    ...
  	    aRow.createCell(countCol).setCellValue(intestatario);
  	    aRow.getCell(countCol).setCellStyle(styleValDef);
  	    
        countCol++;
        aRow.createCell(countCol).setCellValue(item.getStatoFascicolo().getDescrizione());
        aRow.getCell(countCol).setCellStyle(styleValDefAllCenter);

        countCol++;
        String dataStato="";
        if(item.getDataStato()!=null)
        	dataStato = item.getDataStato().format(DateTimeFormatter.ofPattern(pattern));
        aRow.createCell(countCol).setCellValue(dataStato);
        aRow.getCell(countCol).setCellStyle(styleValDefAllCenter);
