...
import it.csi.mudeopen.mudeopensrvsoap.util.Constants;
...
public class OauthHelper {
     ...
     * Gets expires.
     *
      il tempo di expires del token in cache
     */
     ...
     * Gets token.
     *
      il token da usare verso l'API Manager 			<p> 			Se esiste in cache viene tornato l'ultimo token ottenuto dall'API Manager 			altrimenti si effettua la richiesta e si torna il valore ottenuto
     */
     ...
     * Gets new token.
     *
      il token da usare verso l'API Manager 			<p> 			Forza l'ottenimento del token dall'API Manager. Il valore ritornato viene tenuto 			in cache locale per soddisfare successive <code>getToken</code> 			Usare questo metodo solo se e' noto a priori che il valore in cache e' invalido
     */
     ...
     * 
     	il token ottenuto dall'API Manager
     */
    private String getTokenInternal() {
       ...
       expires = responseBody.getExpires_in();
    	...
    	String out = "token di accesso apiman[" + counter + "] " + Util.maskForLog(token) +
    			     " expires in " +  expires;
