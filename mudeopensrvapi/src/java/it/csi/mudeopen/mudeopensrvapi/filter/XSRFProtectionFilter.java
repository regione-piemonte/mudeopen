...
public class XSRFProtectionFilter implements Filter {
	 ...
	 * 
	 * @param hreq
	  true se il path rappresenta una richiesta alla parte REST dell'applicazione
	 * che per default deve contenere la stringa "rest"
	 */
	private boolean isRestRequest(HttpServletRequest hreq) {
		return hreq.getRequestURI() != null && hreq.getRequestURI().contains("rest");
	}
	 ...
	 * 
	 * @param session
	  il token XSRF presente in sessione
	 */
	 ...
	 * 
	 * @param hreq
	  l'header XSRF se presente nella request
	 */
	 ...
	 * 
	 * @param hreq la request
	  il valore del cookie XSRF, se presente nella request; 
	 ...
	 * 
	 * @param session
	  true se in sessione e' presente un token valido
	 */
